...
def get_question_answering_dataset(dataset_name: str):
    if dataset_name == "SQuADv1":
        squad_dataset = load_dataset('squad')
        train_dataset = squad_dataset['train']
        val_dataset = squad_dataset['validation']

        instruction = "Based on the given context, provide only one accurate answer to the question. Follow these rules:\nDo not include multiple options or explanations.\nAnswer in a single sentence, and do not add any extra text after the answer.\n\n"
...
def get_calibration_question_answering_dataset(dataset, sample_num: int):
    if len(dataset['train_dataset']) < sample_num:
        sample_num = len(dataset['train_dataset'])
        print(f"The number of samples in {dataset['name']} dataset is less than the requested sample number. Using {sample_num} samples instead.")
    ...
    def format_sample(sample):
        ...
        if sample['answers']['text']:
            sample['label'] = sample['answers']['text'][0]
        else:
            sample['label'] = 'unanswerable'
        return sample
...
def get_calibration_dataset(dataset_type: str, dataset_names: list, sample_num: int):
    ...
    for name in dataset_names:
        if dataset_type == "classification":
            ...
            dataset = get_question_answering_dataset(name)
            calibration_dataset = get_calibration_question_answering_dataset(dataset, sample_num=sample_num)
