...
NUM_LAYER = int(os.environ.get("NUM_LAYER", 0))

modified_layer_str = os.environ.get('MODIFIED_LAYER', None)
modified_layer = []

modified_layer = list(map(int, modified_layer_str.split()))

logger = get_logger(__name__)

aug_indices = []

# You need to adjust according to the number of players.
def map_index_to_range(index):
    return list(range(index * 4, (index + 1) * 4))

# You need to adjust according to the size of the model.
def get_aug_indices():
    global aug_indices
    aug_indices = aug_indices_comb_harsanyi_classification
    for layer_idx in range(32):
        mapped_sublist = []
        for head_num in aug_indices[layer_idx]:
            mapped_sublist.extend(map_index_to_range(head_num))
        aug_indices[layer_idx] = mapped_sublist

def get_combination_indices(num):
    if num < 0 or num > 255:
        return []
...
def atten_process_eval(attention_map, index=None):
    beta = BETA
    threshod = THRES
    head_indices = aug_indices[index]
    for head_num in head_indices:
        ...
        if indices.numel() != 0 and indices[0][0] == 0:
            indices = indices[1:]
...
def atten_process_cal(key_states, head_comb=None):
    comb_indices = get_combination_indices(head_comb)
    head_indices = []
...
def atten_aug_forward_cal_llama(
    ...
    **kwargs,
    ...
    ##########Modify########
    if signal == NUM_LAYER:
        key_states = atten_process_cal(key_states, head_comb=head_comb)
    ########################
...
def atten_aug_forward_cal_mistral(
    self,
    hidden_states: torch.Tensor,
    attention_mask: Optional[torch.Tensor] = None,
    position_ids: Optional[torch.LongTensor] = None,
